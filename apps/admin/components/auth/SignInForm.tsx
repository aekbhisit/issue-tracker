"use client";
import Checkbox from "@/components/form/inputs/CheckboxInput";
...
import type { LoginCredentials, AuthError } from '@/lib/auth/auth';
...
export default function SignInForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(authError.message || t('admin.auth.loginFailed'));
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
              <button className="inline-flex items-center justify-center gap-3 py-3 text-sm font-medium text-gray-700 transition-colors bg-gray-100 rounded-lg px-4 hover:bg-gray-200 hover:text-gray-800 dark:bg-white/5 dark:text-white/90 dark:hover:bg-white/10 border border-gray-200 dark:border-gray-700">
                ...
                <span suppressHydrationWarning>{t('admin.auth.signInWithGoogle')}</span>
              </button>
              <button className="inline-flex items-center justify-center gap-3 py-3 text-sm font-medium text-gray-700 transition-colors bg-gray-100 rounded-lg px-4 hover:bg-gray-200 hover:text-gray-800 dark:bg-white/5 dark:text-white/90 dark:hover:bg-white/10 border border-gray-200 dark:border-gray-700">
                <svg
                  width="21"
                ...
                <span suppressHydrationWarning>{t('admin.auth.signInWithX')}</span>
              </button>
          </div>
          ...
          <form onSubmit={handleSubmit}>
              <div className="space-y-6">
                <div>
                  <Label>
                    {t('common.label.email')} <span className="text-error-500">*</span>{" "}
                  </Label>
                  <Input
                    placeholder={t('admin.auth.enterEmail')}
                    type="email"
                    value={email}
                    onChange={setEmail}
                    required
                ...
                <div>
                  <Label>
                    {t('common.label.password')} <span className="text-error-500">*</span>{" "}
                  </Label>
                  <div className="relative">
                    <Input
                      type={showPassword ? "text" : "password"}
                      placeholder={t('admin.auth.enterPassword')}
                      value={password}
